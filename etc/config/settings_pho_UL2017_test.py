...
# flag to be Tested
flags = {
    'test'                 : '((ph_r9 >= 0.9) && (ph_hoe <= 0.15) && (ph_sieie <= 0.014) && (ph_ecalPFClusterIso <= ((5.0) +0.01*(ph_et))) && (ph_hcalPFClusterIso <= 12.5 +0.0000003*(ph_et)*(ph_et)) && (ph_trkSumPtHollowConeDR03 <= 6.0 +0.002*(ph_et)))',
    'DNN1_test'                 : '(DNN1 > 0.127)',
    'DNN2_test'                 : '((DNN2 > 0.473) && (ph_sc_abseta < 1.4442)) || ((DNN2 > 0.130) && (ph_sc_abseta > 1.566))',
...
baseOutDir = 'results/UL2017/2023-07-21/'
...
samplesDef = {
    'data'   : tnpSamples.UL2017['data_Run2017BCDEF'].clone(),
...
weightName = 'weights_2017_runBCDEF.PUweight'
if not samplesDef['mcNom' ] is None: samplesDef['mcNom' ].set_weight(weightName)
if not samplesDef['mcAlt' ] is None: samplesDef['mcAlt' ].set_weight(weightName)
if not samplesDef['tagSel'] is None: samplesDef['tagSel'].set_weight(weightName)
if not samplesDef['mcNom' ] is None: samplesDef['mcNom' ].set_puTree('/cms/ldap_home/taebh/CMSSW_11_2_0/src/egm_tnp_analysis/PU_Trees/DY_madgraph_pho.pu.puTree.root')
if not samplesDef['mcAlt' ] is None: samplesDef['mcAlt' ].set_puTree('/cms/ldap_home/taebh/CMSSW_11_2_0/src/egm_tnp_analysis/PU_Trees/DY_amcatnloext_pho.pu.puTree.root')
if not samplesDef['tagSel'] is None: samplesDef['tagSel'].set_puTree('/cms/ldap_home/taebh/CMSSW_11_2_0/src/egm_tnp_analysis/PU_Trees/DY_madgraph_pho.pu.puTree.root')
...
#    { 'var' : 'ph_sc_eta' , 'type': 'float', 'bins': [-2.5,-2.0,-1.566,-1.4442, -0.8, 0.0, 0.8, 1.4442, 1.566, 2.0, 2.5] },
   { 'var' : 'ph_sc_eta' , 'type': 'float', 'bins': [-1.4442,0.0,1.4442] },
   { 'var' : 'ph_et' , 'type': 'float', 'bins': [70,75,80,85,90,100,120,150,200,1000] },
#    { 'var' : 'ph_et' , 'type': 'float', 'bins': [200,225,250,300,400,1000] },
]
...
# cutBase   = 'ph_r9 >= 0.9 && ph_hoe <= 0.15 && ph_sieie <= 0.014 && ph_ecalPFClusterIso <= ((5.0) +0.01*(ph_et)) && ph_hcalPFClusterIso <= 12.5 +0.0000003*(ph_et)*(ph_et) && ph_trkSumPtHollowConeDR03 <= 6.0 +0.002*(ph_et) && tag_Ele_pt > 35 && abs(tag_sc_eta) < 2.17'
cutBase   = '((ph_r9 >= 0.9) && (ph_hoe <= 0.15) && (ph_sieie <= 0.014) && (ph_ecalPFClusterIso <= ((5.0) +0.01*(ph_et))) && (ph_hcalPFClusterIso <= 12.5 +0.0000003*(ph_et)*(ph_et)) && (ph_trkSumPtHollowConeDR03 <= 6.0 +0.002*(ph_et)))'
