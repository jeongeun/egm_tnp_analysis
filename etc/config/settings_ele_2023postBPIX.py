...
baseOutDir = 'results/2023/tnpEleID/afterEGMTalk_newfit/postBPIX_v0_abseta/'
...
samplesDef = {
    'data'   : tnpSamples.Run2023_130X_ReReco_postBPIX['data_Run2023D'].clone(),
    'mcNom'  : tnpSamples.Run2023_130X_ReReco_postBPIX['DY_amc_postBPIX'].clone(),
    'mcAlt'  : tnpSamples.Run2023_130X_ReReco_postBPIX['DY_pow_postBPIX'].clone(),
#    'mcAlt'  : tnpSamples.Run2023_130X_ReReco_postBPIX['DY_mg_postBPIX'].clone(),
   'tagSel' : tnpSamples.Run2023_130X_ReReco_postBPIX['DY_amc_postBPIX'].clone(),
}

## can add data sample easily
#samplesDef['data'].add_sample( tnpSamples.Run2023_130X_ReReco_postBPIX['data_Run2022D'] )
...
weightName = 'weights_data_Run2023D.totWeight' #'weights_2022_runBCD.totWeight' 
if not samplesDef['mcNom' ] is None: samplesDef['mcNom' ].set_weight(weightName)
if not samplesDef['mcAlt' ] is None: samplesDef['mcAlt' ].set_weight(weightName)
if not samplesDef['tagSel'] is None: samplesDef['tagSel'].set_weight(weightName)
if not samplesDef['mcNom' ] is None: samplesDef['mcNom' ].set_puTree('PU_Trees/DY_amc_postBPIX_ele.pu.puTree.root')
#if not samplesDef['mcAlt' ] is None: samplesDef['mcAlt' ].set_puTree('PU_Trees/DY_mg_postBPIX_ele.pu.puTree.root')
if not samplesDef['mcAlt' ] is None: samplesDef['mcAlt' ].set_puTree('PU_Trees/DY_pow_postBPIX_ele.pu.puTree.root')
if not samplesDef['tagSel'] is None: samplesDef['tagSel'].set_puTree('PU_Trees/DY_amc_postBPIX_ele.pu.puTree.root')

#PU_Trees/DY_pow_postBPIX_ele.pu.puTree.root
...
#   { 'var' : 'el_sc_eta' , 'type': 'float', 'bins': [-2.5,-1.566,-1.4442, 0.0, 1.4442, 1.566, 2.5] }, #original
#   { 'var' : 'el_sc_eta' , 'type': 'float', 'bins': [-2.5,-1.566, 0.0, 1.566, 2.5] },
#   { 'var' : 'el_sc_eta' , 'type': 'float', 'bins': [-1.5, 0.0] },
#   { 'var' : 'el_pt' , 'type': 'float', 'bins': [10,20,35,50,100,200,500] },
#   { 'var' : 'el_pt' , 'type': 'float', 'bins': [35,40,45,50,60,70,80,90,100,200,300,1000] }, #original
#   { 'var' : 'el_pt' , 'type': 'float', 'bins': [35,1000] }, #for hole
...
#cutBase   = '(-1.2 < el_sc_phi && el_sc_phi < -0.8) && (el_sc_eta <0 && -1.5 < el_sc_eta) &&  tag_Ele_pt > 35 && el_pt > 35 && abs(tag_sc_eta) < 1.4442  && el_q*tag_Ele_q < 0  && (abs(el_sc_eta) > 1.566 || abs(el_sc_eta) < 1.442)'
#cutBase   = '(-1.2 >= el_sc_phi || el_sc_phi >= -0.8) && (el_sc_eta >= 0 || -1.5 >= el_sc_eta) &&  tag_Ele_pt > 35 && el_pt > 35 && abs(tag_sc_eta) < 1.4442  && el_q*tag_Ele_q < 0  && (abs(el_sc_eta) > 1.566 || abs(el_sc_eta) < 1.442)'
