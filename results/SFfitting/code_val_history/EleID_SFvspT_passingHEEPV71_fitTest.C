void EleID_SFvspT_passingHEEPV71_fitTest()
{
    ...
    TF1 *fitFuncExp = new TF1("fitFuncExp", "[0]*exp(-[1]*x)+[2]", 35, 500);
    fitFuncExp->SetParameters(1, 0.01, 1); 
    gre->Fit(fitFuncExp, "R"); 

    fitFuncExp->SetLineColor(kRed);
    fitFuncExp->SetLineStyle(1);
    fitFuncExp->Draw("same");

    // First-order polynomial fit (linear fit)
    TF1 *fitFuncLin = new TF1("fitFuncLin", "[0] + [1]*x", 35, 300);
    gre->Fit(fitFuncLin, "R");

    fitFuncLin->SetLineColor(kBlue);
    fitFuncLin->SetLineStyle(2);
    ...
    Double_t discrepancy1 = sqrt(pow(parExp[1] - parLin[1], 2) + pow(errExp[1], 2) + pow(errLin[1], 2));
    ...
    TString textExp0 = Form("Exp: c = %.3f #pm %.3f", parExp[2], errExp[2]);
    TString textExp1 = Form("Exp: b = %.3f #pm %.3f", parExp[1], errExp[1]);
    TString textLin0 = Form("Lin: const = %.3f #pm %.3f", parLin[0], errLin[0]);
    TString textLin1 = Form("Lin: slope = %.3f #pm %.3f", parLin[1], errLin[1]);
    TString textDisc0 = Form("Discrepancy (c): %.3f", discrepancy0);
    TString textDisc1 = Form("Discrepancy (slope): %.3f", discrepancy1);

    latex->DrawLatex(0.15, 0.75, textExp0);
    latex->DrawLatex(0.15, 0.70, textExp1);
    latex->DrawLatex(0.15, 0.65, textLin0);
    latex->DrawLatex(0.15, 0.60, textLin1);
    latex->DrawLatex(0.15, 0.55, textDisc0);
    latex->DrawLatex(0.15, 0.50, textDisc1);
...
//    Double_t err2 = fitFunc->GetParError(2);
//
//    TLatex *latex = new TLatex();
//    latex->SetNDC();
...
//    latex->SetTextAlign(13);
//
//    TString text0 = Form("a = %.3f #pm %.3f", par0, err0);
//    TString text1 = Form("b = %.3f #pm %.3f", par1, err1);
//    TString text2 = Form("c = %.3f #pm %.3f", par2, err2);
//
//    latex->DrawLatex(0.15, 0.75, text0);
//    latex->DrawLatex(0.15, 0.70, text1);
//    latex->DrawLatex(0.15, 0.65, text2);
   ...
   42.5, 150, 250, 65, 37.5, 95, 47.5, 55, 650, 75, 85};
   Double_t Graph1_fy1002[11] = {
   0.9770581, 1.003336, 1.012922, 0.9632854, 0.9782076, 1.008109, 0.9785089, 0.9768588, 1.017285, 0.9733013, 1.016067};
   Double_t Graph1_fex1002[11] = {
   2.5, 50, 50, 5, 2.5, 5, 2.5, 5, 350, 5, 5};
   Double_t Graph1_fey1002[11] = {
   0.007176049, 0.008422221, 0.09133076, 0.02725453, 0.005540682, 0.015732, 0.005970407, 0.03078301, 0.0968941, 0.05564281, 0.02091185};
...
//   totopT_up->SetFrameBorderMode(0);
   ...
   0.8972146, 0.9028583, 0.8982836, 0.9008809, 0.87566, 0.9493057, 0.9093868, 0.9108923, 0.922027, 0.9080446, 0.9479908};
   Double_t Graph0_fex1003[11] = {
   2.5, 50, 50, 5, 2.5, 5, 2.5, 5, 350, 5, 5};
   Double_t Graph0_fey1003[11] = {
   0.00764711, 0.01934677, 0.04716883, 0.01674466, 0.006020812, 0.01889451, 0.02194887, 0.01219408, 0.08025521, 0.01770768, 0.0177313};
   ...
   Double_t Graph2_fx1005[11] = {
   42.5, 150, 250, 65, 37.5, 95, 47.5, 55, 650, 75, 85};
   Double_t Graph2_fy1005[11] = {
   0.897335, 0.9210232, 0.9187152, 0.8754167, 0.872615, 0.9204193, 0.908795, 0.9049258, 0.9251668, 0.8778777, 0.9281007};
   Double_t Graph2_fex1005[11] = {
   2.5, 50, 50, 5, 2.5, 5, 2.5, 5, 350, 5, 5};
   ...
   Double_t Graph3_fx1006[11] = {
   42.5, 150, 250, 65, 37.5, 95, 47.5, 55, 650, 75, 85};
   Double_t Graph3_fy1006[11] = {
   0.897335, 0.9210232, 0.9187152, 0.8754167, 0.872615, 0.9204193, 0.908795, 0.9049258, 0.9251668, 0.8778777, 0.9281007};
   Double_t Graph3_fex1006[11] = {
   2.5, 50, 50, 5, 2.5, 5, 2.5, 5, 350, 5, 5};
   ...
   totopT->cd();
    ...
    leg->SetFillColor(0);
    leg->SetFillStyle(0);
    leg->AddEntry("Graph0", "0.000 #leq | #eta | #leq  1.444 (Data)", "PL");
    leg->AddEntry("Graph1", "1.566 #leq | #eta | #leq  2.500 (MC)", "PL");
    leg->Draw();
...
//   0.990136, 0.9811934, 0.9574642, 0.9907405, 0.9912889, 1.021742, 0.9911529, 0.9908023, 0.9837915, 1.001784, 1.041944};
//   Double_t Graph0_fex1001[11] = {
//   2.5, 50, 50, 5, 2.5, 5, 2.5, 5, 350, 5, 5};
//   Double_t Graph0_fey1001[11] = {
//   0.008439095, 0.02102536, 0.05027641, 0.01841488, 0.006815846, 0.02033625, 0.02392237, 0.01326383, 0.08563132, 0.01953569, 0.01948861};
//   TGraphErrors *gre = new TGraphErrors(11,Graph0_fx1001,Graph0_fy1001,Graph0_fex1001,Graph0_fey1001);
//   gre->SetName("Graph0");
//   gre->SetTitle("Graph");
//   gre->SetFillStyle(1000);
