...
// ------------>Primitives in pad: totopT_do
   ...
   Graph_Graph01001->SetMinimum(0.7);
...
//////////////////////// Fitting EE
   ...
   //Double_t systematicUncertainty = sqrt(pow(errConst, 2) + discrepancy);
   ...
   TString textConst    = Form("EE Const Fit: SF = %.3f #pm (95%% CI: %.3f)", parConst,  ci95);
   ...
   //TString textConst  = Form("EE Const Fit: SF = %.3f #pm (95%% CI: %.3f)", parConst,  ci95);
   //TString textLin    = Form("EE Lin Fit: const = %.3f #pm %.3f, slope = %.7f #pm %.7f", parLin[0], errLin[0], parLin[1], errLin[1]);
   //TString textDisc   = Form("EE Discrepancy: %.3f", discrepancy);
   //TString textSysUnc = Form("EE Systematic Uncertainty: %.3f", systematicUncertainty);
   //
   //latex->SetTextColor(kGray+2);
   //latex->DrawLatex(0.15, 0.90, textConst);
   //latex->SetTextColor(kBlue-9);
   ...
   //latex->SetTextColor(kGray+2);
   //latex->DrawLatex(0.15, 0.80, textSysUnc);
   //latex->DrawLatex(0.15, 0.75, textDisc);
...
//////////////////////// Fitting EE
   ...
   //Double_t systematicUncertainty_EB = sqrt(pow(errConst_EB, 2) + discrepancy_EB);
   ...
   TString textConst_EB    = Form("EB Const Fit: SF = %.3f #pm (95%% CI: %.3f)", parConst_EB,  ci95_EB);
   //TString textLin_EB      = Form("EB Lin Fit: const = %.3f #pm %.3f, slope = %.7f #pm %.6f", parLin_EB[0], errLin_EB[0], parLin_EB[1], errLin_EB[1]);
   ...
   //TString textSysUnc_EB   = Form("EB Systematic Uncertainty: %.3f", systematicUncertainty_EB);
   ...
   latex->SetTextColor(kBlack);
   //latex->DrawLatex(0.15, 0.40, textSysUnc_EB);
   //latex->DrawLatex(0.15, 0.35, textDisc_EB);
...
// ------------>Primitives in pad: totopT_up
   ...
   totopT->SaveAs("EleID_SFvspT_passingHEEPV71_2022preEE_v1.pdf");
